...
module BrainpostVideo
  class Movie
    ...
    def initialize(path)
      ...
      command = [BrainpostVideo.ffprobe_binary, '-i', path, '-print_format', 'json', '-show_format', '-show_streams', '-show_error']
      std_output = ''
      std_error = ''
      ...
      if nil_or_unsupported.call(video_stream) && nil_or_unsupported.call(audio_stream)
        @invalid = true
      end
    ...
    def head(location = @path, limit = BrainpostVideo.max_http_redirect_attempts)
      url = URI(location)
      ...
      when Net::HTTPRedirection then
        raise BrainpostVideo::HTTPTooManyRequests if limit == 0
